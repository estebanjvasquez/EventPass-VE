...
import { ArrowLeft, Building2, ClipboardList, Handshake, Pencil, Plus, Receipt, Save, Trash2 } from 'lucide-react'
...
type Org = ActiveOrg
type Option = { id: string; name: string; kind?: string; event_id?: string | null }
...
export default function PatrocinantesGestionAdmin() {
  ...
  const [activeTab, setActiveTab] = useState<'sponsors' | 'packages' | 'companies'>('sponsors')
  ...
  const [scopesByEvent, setScopesByEvent] = useState<Map<string, ProgramScope[]>>(new Map())
  ...
  const load = useCallback(async () => {
    ...
    const [eventsResult, companiesResult, packagesResult, sponsorsResult] = await Promise.all([
      ...
      supabase.from('companies').select('id,name,kind,event_id').eq('organization_id', orgId).order('name'),
    ...
    setScopesByEvent(await loadProgramScopes(orgId, (eventsResult.data ?? []).map(event => event.id)))
    setLoading(false)
  }, [])
  ...
  const compatibleCompanies = (eventId: string) => {
    const sharedEventIds = new Set([eventId, ...(scopesByEvent.get(eventId) ?? []).flatMap(scope => scope.eventIds)])
    return companies.filter(company => company.kind !== 'exhibitor' || sharedEventIds.has(company.event_id ?? ''))
  }
  const scopeNotice = sponsorDraft ? scopesByEvent.get(sponsorDraft.event_id) ?? [] : []
  function newSponsor() {
    setSponsorDraft({ id: '', event_id: events[0]?.id ?? '', company_id: compatibleCompanies(events[0]?.id ?? '')[0]?.id ?? '', package_id: null, status: 'prospect', agreed_amount: null, currency: 'USD', payment_status: 'unpaid', payment_notes: null, additional_amount: 0, agreement_details: null, advertising_contribution: null, print_requirements: null, fulfillment_notes: null })
  }
