-- El superadministrador puede mantener cualquier expositor desde su consola,
-- incluso si la empresa fue creada en otra organización histórica. Los
-- administradores normales siguen limitados a la organización del evento.
...
declare
  ...
  event_org_id uuid;
  is_platform boolean := public.is_platform_admin();
begin
  ...
  if not (is_platform or public.is_org_member(event_org_id)) then
    raise exception 'No tienes permisos para administrar este perfil' using errcode = '42501';
  end if;

  if not exists (select 1 from public.companies c where c.id = p_company_id)
     or (not is_platform and not exists (
