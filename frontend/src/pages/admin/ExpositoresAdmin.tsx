...
export default function ExpositoresAdmin() {
  ...
  const load = useCallback(async () => {
    ...
    const [firmResult, standResult] = await Promise.all([
      supabase.from('companies').select('id,name,contact_name,contact_email,contact_phone,legal_name,tax_id,fiscal_address,billing_email,billing_phone,billing_contact,website,profile_notes').eq('organization_id', event.organization_id).eq('kind', 'exhibitor').order('name'),
