  ...
  return <section className="mt-6 space-y-6">
    ...
    <div className="grid items-start gap-6 xl:grid-cols-[minmax(300px,400px)_minmax(0,1fr)]">
      <form onSubmit={e => { e.preventDefault(); void save(); }} className="space-y-5 rounded-2xl border border-zinc-200 bg-white p-5">
        ...
        {draft.ticker_animated === false && <Field title="Alineación sin movimiento"><select className={fieldClass} value={draft.ticker_static_align ?? 'center'} onChange={e => change('ticker_static_align', e.target.value as 'left' | 'center')}><option value="center">Centrado</option><option value="left">A la izquierda</option></select></Field>}
