...
type Company = { id: string; name: string; contact_name: string | null; contact_email: string | null; contact_phone: string | null; legal_name: string | null; tax_id: string | null; fiscal_address: string | null; billing_email: string | null; billing_phone: string | null; billing_contact: string | null; website: string | null; profile_notes: string | null; public_logo_url?: string | null; public_description?: string | null; public_category?: string | null; public_profile_status?: string | null }
...
export default function ExpositoresAdmin() {
  ...
  const load = useCallback(async () => {
    ...
    const [firmResult, standResult] = await Promise.all([
      supabase.from('companies').select('id,name,contact_name,contact_email,contact_phone,legal_name,tax_id,fiscal_address,billing_email,billing_phone,billing_contact,website,profile_notes,public_logo_url,public_description,public_category,public_profile_status').eq('organization_id', event.organization_id).eq('kind', 'exhibitor').order('name'),
  ...
  async function reviewPublicProfile(companyId: string, approved: boolean) {
    const { error: reviewError } = await supabase.rpc('review_exhibitor_public_profile', { p_company_id: companyId, p_approved: approved })
    if (reviewError) setError(reviewError.message); else { setNotice(approved ? 'Perfil aprobado y visible en el plano.' : 'Perfil devuelto para correcciones.'); await load() }
  }
  ...
  return <div className="min-h-[100dvh] bg-zinc-50">
    ...
    <main className="mx-auto max-w-5xl px-5 py-8"><h1 className="text-2xl font-bold">Expositores{eventName ? ` · ${eventName}` : ''}</h1><p className="mt-1 text-sm text-zinc-600">Gestiona empresas y asigna uno o varios espacios desde una vista comercial separada del diseño.</p>
      ...
      <section className="mt-6 rounded-xl border bg-white p-4"><h2 className="font-semibold">Revisión de perfiles públicos</h2><p className="mt-1 text-xs text-zinc-600">Aprueba el branding que los expositores enviaron desde su portal.</p><div className="mt-3 space-y-2">{companies.filter((company) => company.public_profile_status === 'pending' || company.public_profile_status === 'approved').map((company) => <div key={company.id} className="flex flex-wrap items-center justify-between gap-3 rounded-lg bg-zinc-50 p-3 text-sm"><span><b>{company.name}</b><span className="ml-2 text-xs text-zinc-500">{company.public_profile_status === 'approved' ? 'Aprobado' : 'Pendiente'}{company.public_category ? ` · ${company.public_category}` : ''}</span></span><div className="flex gap-2">{company.public_profile_status !== 'approved' && <button type="button" onClick={() => void reviewPublicProfile(company.id, true)} className="rounded-lg bg-emerald-700 px-3 py-1.5 text-xs font-semibold text-white">Aprobar</button>}{company.public_profile_status === 'approved' && <button type="button" onClick={() => void reviewPublicProfile(company.id, false)} className="rounded-lg border border-amber-300 px-3 py-1.5 text-xs font-semibold text-amber-800">Solicitar cambios</button>}</div></div>)}{!companies.some((company) => company.public_profile_status === 'pending' || company.public_profile_status === 'approved') && <p className="text-sm text-zinc-500">No hay perfiles enviados.</p>}</div></section>
    </main>
  </div>
}
